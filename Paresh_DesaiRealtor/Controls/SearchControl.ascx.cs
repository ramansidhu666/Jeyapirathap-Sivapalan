...
namespace Property.Controls
{
    public partial class SearchControl : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (Request.QueryString["City"] != null && Request.QueryString["City"] != "")
                {
                    ...
                    if(Type=="Residential")
                    {
                        SearchByCities("GetPropertiesByCities_Residential");
                    }
                    else if (Type == "Commercial")
                    {
                        SearchByCities("GetPropertiesByCities_Commercial");
                    }
                    else
                    {
                        SearchByCities("GetPropertiesByCities_Condo");
                    }
                    
                    Session["SearchMLS"] = null;
                }
                else
                {
        ...
        public void SearchByCities(string Type)
        {
            DataTable dt = new DataTable();
            string constr = ConfigurationManager.ConnectionStrings["ServiceDataBase"].ConnectionString;
            ...
            cmd.Parameters.AddWithValue("@MLSID_City_PostalCode", Session["City"].ToString());
            cmd.Parameters.AddWithValue("@PropertyType", (Session["HomeType"] == null || Session["HomeType"] == "" ? "0" : Session["HomeType"].ToString()));
            cmd.Parameters.AddWithValue("@MinPrice", (Session["MinPrice"] == null || Session["MinPrice"] == "" ? "0" : Session["MinPrice"].ToString()));
            cmd.Parameters.AddWithValue("@MaxPrice", (Session["MaxPrice"] == null || Session["MaxPrice"] == "" ? "0" : Session["MaxPrice"].ToString()));
            cmd.Parameters.AddWithValue("@BedRooms", (Session["Beds"] == null || Session["Beds"] == "" ? "0" : Session["Beds"].ToString()));
            cmd.Parameters.AddWithValue("@BathRooms", (Session["Baths"] == null || Session["Baths"] == "" ? "0" : Session["Baths"].ToString()));
            cmd.Parameters.AddWithValue("@SaleLease", (Session["SaleLease"] == null || Session["SaleLease"] == "" ? "0" : Session["SaleLease"].ToString()));
            cmd.Parameters.AddWithValue("@MLS", (Session["SearchMLS"] == null || Session["SearchMLS"] == "" ? "" : Session["SearchMLS"].ToString()));
